...
define("TMPLFILE_LIST",   "admin_client_search.tmpl");
define("OPERATION_SEARCH", "Search client");
define("DELETE_SEARCH",    "Delete client");
...
{ 
    ...
    foreach ($hosts as $host) {
        ...
        $looptag[$i]["<<MAC>>"] = $pieces[1];
        $looptag[$i]["<<IP>>"] = $pieces[2];
...
{ 
    ...
    foreach ($hosts as $host) {
        ...
        if ($post["macaddr"] != "") {
            /* 一致なら */
            if ($post["macsearch"] == "same"){
                $macaddr = check_macaddr($post["macaddr"]);
                if ($macaddr != $mac) {
                    continue;
                }
            /* 含むなら */
            } else {
                if (strpos($mac, strtolower($post["macaddr"])) === FALSE) {
                    continue;
                }
            }
        }
...
if (isset($_POST["search"]) || isset($_POST["download"])) {
    ...
    $in_mac = escape_html($_POST["macaddr"]);
    $select_mac = $_POST["macsearch"];
    $in_ip = escape_html($_POST["ipaddr"]);
    $in_case = $_POST["ipselect"];
    /* 入力値の入力チェック */
    $ret = check_search_in($in_host, $in_mac, $in_ip);
    switch ($ret) {
    /* 入力値エラーの場合 */
    case 1:
    ...
    /* MACアドレスの入力チェック */
    case 2:
        $err_msg = sprintf($msgarr['32002'][SCREEN_MSG]);
        $log_msg = $msgarr['32002'][LOG_MSG];
        result_log(OPERATION_SEARCH . ":NG:" . $log_msg, LOG_ERR);
        break;
    /* IPアドレスの入力チェック */
    case 3:
        $err_msg = sprintf($msgarr['32003'][SCREEN_MSG]);
        $log_msg = $msgarr['32003'][LOG_MSG];
        result_log(OPERATION_SEARCH . ":NG:" . $log_msg, LOG_ERR);
        break;
    /* 正常の場合 */
    case 0:
            ...
            if (is_array($_SESSION[STR_IP]["$sn"])) {
                foreach ($_SESSION[STR_IP]["$sn"] as $sub => $value) {
                    if (isset($_SESSION[STR_IP]["$sn"]["$sub"]["host"])) {
                        ...
                        /* 条件検索の関数を呼ぶ */
                        $search_result = search_client($sn, $sub, $hostline, $_POST);
                        $search_result_array = array_merge($search_result_array, $search_result);
                    }
                }
            }
        }
        ...
        if (isset($_POST["search"])) {
            ...
            client_re_display($in_case, $select_host, $select_mac);
        /* 検索結果ダウンロードボタンの時 */
